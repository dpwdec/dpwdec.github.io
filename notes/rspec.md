...
---
### Pending Tests
You can define pending tests in your spec files by using the `pending` keyword in place of an `it` for the thing you test should do. This prints messages telling you which tests are not yet implemented. Very useful for the planning stage and making sure you have an indication of what tests you might write in the future. `pending` tests are **not blocks** you cannot place code in them.
```ruby
...
2. Create an environment variable that tells our application whether we are using the test database or the live database (or some other 
...
[Guard][gd1] is a framework for automatically running RSpec tests when files inside the directory change (i.e. are saved). You install guard by adding it to your `Gemfile` in the `development` group and then running the `bundle` command.
```ruby
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMzM0MTE2MTYzLC0xNjgwNjUxMTgsMTY4Nz
