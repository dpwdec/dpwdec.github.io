...
x = 6
```
You can create a constant with the `const` keyword. Constants are *always* immutable and cannot be used with the `mut` keyword. Furthermore constants can *never* be changed or assigned using by functions that require computation at run time.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE0MTQ5ODA0NDQsLTE2MzgyMzI2NzcsMj
