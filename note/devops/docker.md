...
You can **start a container that has stopped** using the `start` command. You can use the container name here or the id of the container that is listed under `CONTAINER ID` when the `ps` command is used.
```bash
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTUwMDY3NjUwNiwxNzUzMTI5ODkyLDYxNz
