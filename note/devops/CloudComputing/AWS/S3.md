...
You can **access objects from S3** using the `getObjects` method on the `S3` object. This is an asynchronous function that takes an object `Bucket` property that describes the name of the bucket, and a `Key` property which is a path to the object you want to access. You **must convert the result of `getObjects` into a promise** to be able to handle it with `await` or other asynchronous protocols.
```js
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTUxMTU2ODk1NywyMTQzNTk1Nzk5LC0xMD
Y3NTQ3ODQ5LC00MjYyMDY4MzYsLTE3ODA0MzE0NTcsMzcxNDQ5
Nzc3LC0xMzIzNzYwNDEwLDg3MzE5NjUxMCwxNjY2OTgzMTUzLC
0xNzY2OTQxNDQzLDEwNjUwMjE0MjQsLTg3ODE4MDYxLDY0Nzc2
MTM4LDIwODkzODMyMCwtOTQwNDg2MjE2LDE2ODAwMTQ5OTYsLT
IxMjI2NDU0MDQsLTE5MTE5ODMwN119
-->