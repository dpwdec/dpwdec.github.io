...
You can **remove white space and new line generation** between jinja if statements that don't get templated by adding `-` signs before or after the `{% %}` statement markers to remove different amounts of white space. The example below will strip out all newline and blank spaces when the if statement is not templated.
```
{% if number % 2 == 0 -%}
Data: Even
{% end if -%}
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTYyMzYyNTcyNiwtMTUzOTM0MTk1NywtMT
AyOTY5Nzk4MF19
-->