...
You can **create links between pages** by using the `Link` component from `react-router-dom`. The `Link` component should take a `to` element as a `prop` that is assigned to the page extension you want to link to. This allows you to add links between page extension in a traditionally structure manner.
```js
